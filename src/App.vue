...
export default {
  ...
  mounted: async function load() {
    try {
      ...
      this.tockens = Object.values(data.Data);
  ...
  computed: {
    ...
    paginatedTickers() {
      return this.filteredTickers.slice(this.startIndex, this.endtIndex);
    },

    hasNextPage() {
      return this.endtIndex < this.filteredTickers.length;
    },
    normalizedGraph() {
  ...
  methods: {
    subscribeToUpdate(name) {
      ...
      const sub = setInterval(async () => {
        const res = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${name}&tsyms=USD&api_key=352e5d9d63e9b6f5b32982332a48732392769dab52baa2eefd26b2041ad4a633`,
        );
    ...
    add(name) {
      if (this.tickers.find((t) => t.name === name.trim())) {
        this.isError = true;
        this.ticker = name;
        return;
      }
    ...
    remove(id) {
      this.tickers = this.tickers.filter((t) => t !== id);
      // localStorage.setItem('cryptonomicon-list', JSON.stringify(this.tickers));
    ...
    setTip() {
      this.isError = false;
      this.tips = [];
      for (let t of this.tockens) {
        if (this.tips.length >= 4) {
          break;
        }
        if (
          t.Symbol.toLowerCase().startsWith(this.ticker.toLowerCase()) ||
          t.FullName.toLowerCase().startsWith(this.ticker.toLowerCase())
        ) {
          this.tips.push(t);
        }
      }
